		...
		uint8_t length, uint32_t module_id)
{
	...
	ret = wait_event_timeout(this_afe.wait[index], (atomic_read(&this_afe.state) == 0),
		...
		uint32_t param_id, uint32_t module_id)
{
	...
	calib_resp->get_param.payload_size = sizeof(*calib_resp) - sizeof(calib_resp->get_param);
	...
	if (ret < 0) {
		pr_err("[Smartamp:%s] get param port %d param id[0x%x]failed\n", __func__,
		       port_id, calib_resp->get_param.param_id);
		goto fail_cmd;
	}
