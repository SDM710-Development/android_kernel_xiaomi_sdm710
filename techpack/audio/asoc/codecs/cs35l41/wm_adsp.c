...
static int wm_adsp_cal_get(struct snd_kcontrol *kcontrol,
			     struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);
	struct wm_adsp *dsp = snd_soc_codec_get_drvdata(codec);
...
static int wm_adsp_cal_put(struct snd_kcontrol *kcontrol,
			     struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);
	struct wm_adsp *dsp = snd_soc_codec_get_drvdata(codec);
	...
	while ((len - read_len) > 0) {
		...
		if (ret < 0) {
			printk("%s failed, toread_len:%u reg base:%u reg:%u read_len:%u\n",
			       __func__, (unsigned int)toread_len, reg,
			       (unsigned int)(reg + read_len), (unsigned int)read_len);
			return ret;
		}
		read_len += toread_len;
	}
			...
			struct snd_ctl_elem_value *ucontrol)
{
	...
	if (ctl->flags & WMFW_CTL_FLAG_VOLATILE) {
		printk("it is a volatile variable, can't apply its value");
		ret = -EPERM;
	} else
		memcpy(ctl->cache, p, ctl->len);
...
static int wm_adsp_cal_ctl_put(struct wm_adsp *dsp, const char *name, int value)
{
...
static int wm_adsp_cal_ctl_get(struct wm_adsp *dsp, const char *name)
{
	struct snd_kcontrol *kctl = NULL;
	struct snd_ctl_elem_value ucontrol;
	struct snd_soc_card *card = dsp->codec->component.card;
