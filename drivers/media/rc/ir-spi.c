				 ...
				 unsigned long arg)
{
	...
	case LIRC_GET_FEATURES:
		return put_user(idata->lirc_driver.features, (__u32 __user *)arg);
	...
	case LIRC_SET_SEND_MODE: {
		...
		if (idata->xfer.len)
			...
			if (p > idata->buffer_size || idata->buffer == NULL) {
				pr_debug("IR new malloc %d", (int)idata->xfer.len);
...
static int ir_spi_probe(struct spi_device *spi)
{
	...
	snprintf(idata->lirc_driver.name, sizeof(idata->lirc_driver.name), IR_SPI_DRIVER_NAME);
