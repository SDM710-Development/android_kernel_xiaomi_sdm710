...
static void cam_node_recycle_ctxt_from_acquired_list(struct cam_node *node)
{
	struct cam_context *ctx = NULL;
	int rc;
	...
	if (!list_empty(&node->acquired_ctx_list)) {
		ctx = list_first_entry(&node->acquired_ctx_list, struct cam_context, list);
		list_del_init(&ctx->list);
	}
	mutex_unlock(&node->list_mutex);
	...
	CAM_WARN(CAM_CORE, "[%s] Recycling ctx_id=%d, ctx_released=%d", node->name, ctx->ctx_id,
		 ctx->ctx_released);
	...
	if (ctx->ctx_released == false) {
		...
		if (rc)
			CAM_ERR(CAM_CORE, "destroy device handle is failed node %s", node->name);
	}
