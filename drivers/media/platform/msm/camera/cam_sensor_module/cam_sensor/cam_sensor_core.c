	...
	void *arg)
{
	...
	case CAM_ACQUIRE_DEV: {
		...
		CAM_DBG(CAM_SENSOR, "Operation mode: %0x", g_operation_mode);
	...
	case CAM_UPDATE_REG: {
		...
		CAM_DBG(CAM_SENSOR, "CAM_UPDATE_REG reg setting size = %d",
		...
		for (i = 0; i < user_reg_setting.size; i++) {
			CAM_DBG(CAM_SENSOR,
				"CAM_UPDATE_REG reg_addr=0x%x, reg_value=0x%x",
	...
	case CAM_READ_REG: {
		...
		CAM_DBG(CAM_SENSOR, "CAM_READ_REG reg setting size = %d",
			user_reg_setting.size);
		i2c_reg_setting = kzalloc(sizeof(struct cam_sensor_i2c_reg_array) *
					  user_reg_setting.size, GFP_KERNEL);
		if (!i2c_reg_setting) {
		...
		for (i = 0; i < user_reg_setting.size; i++) {
			...
			CAM_DBG(CAM_SENSOR,
				"CAM_READ_REG reg_addr=0x%x, reg_value=0x%x, sid = 0x%x",
				i2c_reg_setting[i].reg_addr,
				i2c_reg_setting[i].reg_data,
				s_ctrl->io_master_info.cci_client->sid);
		}

		if (copy_to_user((void __user *)user_reg_setting.reg_setting,
				 i2c_reg_setting,
				 sizeof(struct cam_sensor_i2c_reg_array) *
				 user_reg_setting.size) || ret != 0) {
