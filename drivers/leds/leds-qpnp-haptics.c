...
static ssize_t qpnp_haptics_show_effect_samp(struct device *dev, struct device_attribute *attr,
		char *buf)
{
	struct led_classdev *cdev = dev_get_drvdata(dev);
	struct hap_chip *chip = container_of(cdev, struct hap_chip, cdev);
	...
	ptr = str;
	for (i = 0; i < HAP_WAVE_SAMP_LEN; i++) {
		len = scnprintf(ptr, HAP_STR_SIZE, "%x ",
...
static ssize_t qpnp_haptics_store_effect_samp(struct device *dev, struct device_attribute *attr,
...
static ssize_t qpnp_haptics_show_effect_max(struct device *dev, struct device_attribute *attr,
		char *buf)
{
	struct led_classdev *cdev = dev_get_drvdata(dev);
	struct hap_chip *chip = container_of(cdev, struct hap_chip, cdev);
...
static ssize_t qpnp_haptics_store_effect_max(struct device *dev, struct device_attribute *attr,
		const char *buf, size_t count)
{
	struct led_classdev *cdev = dev_get_drvdata(dev);
	struct hap_chip *chip = container_of(cdev, struct hap_chip, cdev);
...
static struct device_attribute qpnp_haptics_attrs[] = {
	...
	__ATTR(effect_samp, 0664, qpnp_haptics_show_effect_samp, qpnp_haptics_store_effect_samp),
	__ATTR(effect_max, 0664, qpnp_haptics_show_effect_max, qpnp_haptics_store_effect_max),
};
...
static int qpnp_haptics_parse_buffer_dt(struct hap_chip *chip)
{
	...
	if (!rc) {
		...
		if (prop) {
			if (temp != HAP_WAVE_SAMP_LEN * chip->effect_max) {
				dev_err(&chip->pdev->dev, "Invalid len of effect array\n");
				chip->effect_max = 0;
				return -EINVAL;
			}
