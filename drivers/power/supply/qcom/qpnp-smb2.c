...
static int smb2_parse_dt(struct smb2 *chip)
{
	...
	if (of_find_property(node, "qcom,thermal-mitigation-dcp", &byte_len)) {
		...
		rc = of_property_read_u32_array(node, "qcom,thermal-mitigation-dcp",
						chg->thermal_mitigation_dcp, chg->thermal_levels);
		if (rc < 0) {
	...
	if (of_find_property(node, "qcom,thermal-mitigation-qc3", &byte_len)) {
		...
		chg->thermal_levels = byte_len / sizeof(u32);
		rc = of_property_read_u32_array(node, "qcom,thermal-mitigation-qc3",
						chg->thermal_mitigation_qc3, chg->thermal_levels);
		if (rc < 0) {
			dev_err(chg->dev, "Couldn't read threm limits rc = %d\n", rc);
			return rc;
		}
	...
	if (of_find_property(node, "qcom,thermal-mitigation-qc2", &byte_len)) {
		...
		chg->thermal_levels = byte_len / sizeof(u32);
		rc = of_property_read_u32_array(node, "qcom,thermal-mitigation-qc2",
						chg->thermal_mitigation_qc2, chg->thermal_levels);
		if (rc < 0) {
			dev_err(chg->dev, "Couldn't read threm limits rc = %d\n", rc);
			return rc;
		}
	...
	if (of_find_property(node, "qcom,thermal-mitigation-pd-base", &byte_len)) {
		chg->thermal_mitigation_pd_base = devm_kzalloc(chg->dev, byte_len, GFP_KERNEL);
		if (chg->thermal_mitigation_pd_base == NULL)
			return -ENOMEM;

		chg->thermal_levels = byte_len / sizeof(u32);
		rc = of_property_read_u32_array(node, "qcom,thermal-mitigation-pd-base",
