...
static ssize_t
thermal_message_of_batt_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	if (!tm || !tm->message_ok)
		return -EINVAL;
...
static ssize_t
thermal_temp_state_store(struct device *dev, struct device_attribute *attr, const char *buf,
			 size_t len)
{
	int val = -1;
