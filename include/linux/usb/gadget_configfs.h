...
/* default usb serial number, defined in init.qcom.usb.sh */
...
{							\
	...
	if (strncmp(VNAME(__name), "serialnumber", 12) == 0 && \
	    gs->__name != NULL && strncmp(page, DEFAULT_USB_SERIAL_NUMBER, 7) == 0) { \
		pr_err("usb serialnum is not NULL, don't set to %s\n", page); \
		return -EINVAL;				\
	}						\
							\
